    ...
    <a href={`/product/${slug}`} className="w-full max-w-[520px] h-full bg-white rounded-card relative flex flex-col pb-6">
      ...
      <div className="flex gap-3 mx-[27px] mt-6">
        <button
          ...
          className="w-[62px] h-[62px] shrink-0 border border-surface rounded-btn flex items-center justify-center text-ink"
        >
          <WishlistIcon className={`w-6 h-6 ${isWishlisted ? 'text-brand' : ''}`} filled={isWishlisted} />
        </button>
